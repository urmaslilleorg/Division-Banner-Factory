...
export default function BannerCard({ banner, onClick }: BannerCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Video player — shown for video banners with a video URL */}
        {banner.isVideo && banner.videoUrl && banner.videoUrl.startsWith("data:video") ? (
          <video
            src={banner.videoUrl}
          ...
          /* Preview PNG from render job */
          <div className="relative max-h-full max-w-full">
            <img
              src={banner.videoUrl}
              alt={`Banner ${banner.bannerId} preview`}
              className="max-h-full max-w-full object-contain opacity-80"
            />
            <span className="absolute inset-0 flex items-center justify-center">
              <span className="rounded-full bg-black/50 px-2 py-1 text-[10px] text-white">▶ Video preview</span>
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-1 text-red-300">
            ...
            <span className="text-xs">Video — no preview</span>
          </div>
        ) : (
