...
interface FigmaIntegrationPanelProps {
  ...
  /** Number of non-slide banners in this campaign (for sync warning) */
  bannerCount?: number;
}
...
export function FigmaIntegrationPanel({
  ...
  // ── Task 3: Sync warning ──────────────────────────────────────────────────
  // Simple heuristic: if last sync is older than 24 hours and there are banners,
  // show a "copy may have changed" warning.
  const showSyncWarning = useMemo(() => {
    if (!lastSync || bannerCount === 0) return false;
    try {
  ...
  const activeKey = manualMode ? fileKey : selectedFileKey;
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 space-y-4">
      ...
      {/* Task 6: File selector — dropdown if files available, manual input otherwise */}
      ...
      {showSyncWarning && (
        <div className="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-700 flex items-center gap-2">
          <span>⚠</span>
          <span>
            Copy may have changed since last sync ({formatSync(lastSync)}) — click{" "}
            <strong>Sync to Figma</strong> to update.
          </span>
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={handleSync}
          disabled={syncing || !activeKey.trim()}
